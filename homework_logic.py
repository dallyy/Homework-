...
class Homework:
    ...
    def recommend(self, correct):
        if correct:
            if self.cur_q:sorted(filter(lambda q: q.id != self.cur_q.id, self.q_bank), key=lambda q: abs(q.diff - self.student["rating"]))
            return self.q_bank.pop(0) 
        else:
            return self.cur_q
    ...
    def evaluate_answer(self, q, user_answer):
        try:
            if "x" in q.q:return user_answer == Un.solve_equation(q.q)  
